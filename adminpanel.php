...
require 'db.php';
session_start();
if(isset($_POST['delburger'])){
  $conn -> query("DELETE FROM `burgers` WHERE `id` = '{$_POST['burgerid']}'");
} 
?>
<!DOCTYPE html>
<html lang="ru">
...
<body>
  ...
  <div class="container__style">
        ...
        <span class="login__users" id="login">
<?php
      error_reporting(0);
              require 'db.php';
              session_start();
              ...
              <?php
                  if(isset($_SESSION['users'])) {
                      echo '<li class="btn__style"><a class="logoutstyle" href="out.php">Выйти</a></li>';
                      echo '<li class="btn__style"><a class="logoutstyle" href="index.php">Вернуться</a></li>';
                  } 
              ?>
        ...
        <?php
            if(!isset($_SESSION['users'])) {
                echo '<a class="btnstyle" href="login.php">Войти</a>';
                echo '<a class="btnstyle" href="registration.php">Зарегистрироваться</a>';
                echo '<a class="btnstyle" href="adminpanel.php">Админ панель</a>';
            }
        ?>
  </div>
      ...
      <div class="container navigation__container">
        <ul class="navigation__list">
          <li class="navigation__item">
            <button class="navigation__button navigation__button_burger navigation__button_active">Бургеры</button>
          </li>
          <li class="navigation__item">
            <button class="navigation__button navigation__button_snack">Закуски</button>
          </li>
          <li class="navigation__item">
            <button class="navigation__button navigation__button_hotdog">Хот-доги</button>
          </li>
          <li class="navigation__item">
    ...
    <section class="catalog">
      <div class="container catalog__container">
        <div class="catalog__order order ">
          <form action="add-burger.php" method="POST" enctype="multipart/form-data">
            <aside class="order__wrapper">
            <h3 class="catalog__title">Добавление </h3>
              <input class="modal-delivery__input" type="text" placeholder="Введите название" name="title">
              <input class="modal-delivery__input" type="text" placeholder="Введите цену" name="price">
              <input class="modal-delivery__input" type="text" placeholder="Введите кол-во грамм" name="weight">
              <input class="modal-delivery__input" type="text" placeholder="Введите кол-во калорий " name="calories">
              <input class="modal-delivery__input" type="text" placeholder="Введите описание" name="description">
              <input class="modal-delivery__input" type="text" placeholder="Введите ингредиенты" name="ingredients">
              <select name="category_id" id="category_id" class="modal-delivery__input"> 
                <?php  
                $model = mysqli_query($conn, "SELECT * FROM makes");
                foreach ($model as $keymodel => $valuemodel): ?>
                    <option value="<?=$valuemodel['id']?>"><?=$valuemodel['category']?></option>
                <?php endforeach;?>
              </select>
              <input class="modal-delivery__input" type="file" placeholder="Введите название" name="image">
              <button id="addTovar" name="addTovar" class="product__add">Добавить</button>
            </aside>
          </form>
        </div>
        <div class="catalog__wrapper">
          ...
          <ul class="catalog__list">
            <?php
            $burgers = $conn -> query("SELECT * FROM `burgers`");
            foreach($burgers as $key => $value):
            ?>
            <li class="catalog__item">
              <article class="product">
                <img class="product__image" src="img/img_new/<?=$value['image']?>" alt="Мясная бомба">

                <p class="product__price"><?=$value['price']?><span class="currency">₽</span></p>

                <h3 class="product__title">
                  <button class="product__detail"><?=$value['title']?></button>
                </h3>

                <p class="product__weight"><?=$value['weight']?> г</p>

                <button class="product__add">Редактировать</button>
                <?php
                if(isset($_SESSION['users'])){
                  if($_SESSION['users']['isAdmin'] == 1 ):?>
                  <form action="" method="post">
                    <input type="hidden" name="burgerid" value="<?=$value['id']?>">
                    <button class="product__add" name="delburger">Удалить</button>
                  </form>
                  <?php
                endif;}
                ?>
